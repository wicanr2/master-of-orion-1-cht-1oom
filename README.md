...
科技名、科技描述等文字烘在版權資料檔 `research.lbx` 裡。本引擎分支沒有 mod 覆蓋機制(PBX),又不能散布改過的版權檔。解法是引擎內**載入後覆蓋層** `strtr`:用自帶的 `英文→繁中` 對照表,在字串顯示前即時替換。譯文是我們的衍生資產,版權檔分毫未動。
...
- [~] **密集面板破版修正**:殖民生產列、外交關係框等緊湊版面,24px 中文仍有重疊,逐屏調整中
- [ ] 烘字標籤完整覆蓋(部分畫面 tab/按鈕仍為英文)
- [ ] 外交/新聞訊息(`diplomat.lbx`/`eventmsg.lbx`,含動態 token)
